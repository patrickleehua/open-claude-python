"""Coordinator mode system prompt — ported from Claude-Code-rev coordinatorMode.ts."""

from __future__ import annotations

# Tool name constants used in coordinator prompts
...
def get_coordinator_user_context(
    ...
    *,
    ...
    async_agent_allowed_tools: set[str] | None = None,
    scratchpad_gate_enabled: bool = False,
) -> dict[str, str]:
    """Build the worker-tools context block injected into the user message
    when running in coordinator mode.

    Parameters mirror the dependency-injected values that the TypeScript
    version resolves at call-site from env / feature gates.
    """
    ...
    if simple_mode:
        ...
        allowed = async_agent_allowed_tools or set()
        worker_tools = ", ".join(
            sorted(name for name in allowed if name not in internal_worker_tools)
        )

    content = (
        f"Workers spawned via the {AGENT_TOOL_NAME} tool have access to these tools: {worker_tools}"
    )
    ...
    if scratchpad_dir and scratchpad_gate_enabled:
        content += (
...
def get_coordinator_system_prompt(
    *,
    ...
    The *simple_mode* flag selects between the reduced and full worker
    capability description, mirroring the CLAUDE_CODE_SIMPLE branch in the
    TypeScript original.
    """
    if simple_mode:
        worker_capabilities = (
...
"""Pre-built coordinator prompt using default (non-simple) worker capabilities."""
